...
from guepard_mcp.compute.tools import StartComputeTool, GetComputeStatusTool
...
async def test_start_compute():
    ...
    start_tool = StartComputeTool(client)
    ...
    try:
        # First check status before starting
        print("    📊 Checking status BEFORE start...")
        status_before = await status_tool.execute({"deployment_id": deployment_id})
        print(f"    Status before: {status_before}")
        
        # Start compute
        print("    🚀 Starting compute...")
        result = await start_tool.execute({"deployment_id": deployment_id})
